...
# Load Data
df_EDA <- read.csv("~/Desktop/df_EDA.csv", header = TRUE)
df_PCA <- read.csv("~/Desktop/df_PCA.csv", header = TRUE)
df_PCA$diabetes <- as.factor(df_EDA$diabetes)
df_EDA$diabetes <- as.factor(df_EDA$diabetes)

#Splitting our data up
set.seed(123)
splits <- sample(2, nrow(df_PCA), replace = TRUE, prob = c(0.6, 0.4))
train_pca <- df_PCA[splits == 1, ]
test_pca  <- df_PCA[splits == 2, ]
train_eda <- df_EDA[splits == 1, ]
test_eda  <- df_EDA[splits == 2, ]
...
for (k in 2:6) {
  ...
  set.seed(123)
  split <- sample(2, nrow(temp_df), replace = TRUE, prob = c(0.6, 0.4))
  train <- temp_df[split == 1, ]
  test <- temp_df[split == 2, ]
...
plot(2:6, accuracy_results, type = "b",
